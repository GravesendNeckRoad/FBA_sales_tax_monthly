...
from Utilities.utils import Helpers, Style, ZeroSalesError
...
class GenerateFBAReport:
    ...
    def __init__(self):    
        ...
        try:
            ...
            if isinstance(self.current_accounts, list) and len(self.current_accounts) == 0:
                raise SyntaxError("Empty list detected for ACCOUNTS_LIST - must pass at least one acc name")

        except Exception as e:
            logging.error(f"Issue with environment variable: 'ACCOUNTS_LIST' - {str(e)}")
            raise
    ...
    def __validate_user_input(self, start_date: str, end_date: str) -> Tuple[str, str]:
        ...
        try:            
            ...
            # NOTE bypassing this for now since we are rebuilding the shit to work in loops 
        ...
        while current_attempt <= max_retries:
            try:
                ...
                if token_request.status_code == 200:    
                    ...
                    self.backoff.exponential_backoff(n=current_attempt, rate_of_growth=1.75, base_seconds=4)
                    current_attempt += 1
    ...
    def request_FBA_report(
        ...
        # TODO - this fails if you leave it out for get_report - figure that out 
        if not report_type:
        ...
        while current_attempt <= max_attempts:
            try:                
                ...
                if request_download.status_code == 202:
                    ...
                    logging.error(f"{request_download.status_code} Error for report ID {self.report_id}")
                    Helpers.exponential_backoff(n=current_attempt, rate_of_growth=1.75, base_seconds=4)
                    current_attempt += 1
    ...
    def download_report(self, download_url: Optional[str] = None, compression: Optional[str] = None) -> pd.DataFrame:
        ...
        while attempt <= max_attempts:
            try:
                ...
                if download.status_code in [400, 401, 403, 404]:
                    ...
                    self.backoff.exponential_backoff(n=attempt, rate_of_growth=1.75, base_seconds=4)
                    attempt += 1                                
                else:
        ...
        try:
            ...
            df = pd.read_csv(io.StringIO(report_contents), sep='\t', encoding='latin1', low_memory=False)
            return df       
...
class ReportAssembler:
    ...
    def __init__(self, account_name: Optional[str] = None):
        ...
        self.tax_state_report_name = None
...
class ReportDownloadOrchestrator:
    ...
    def __init__(self, account_name: str):       
        ...
        try:
            ...
            logging.error(
                ...
                f"Make sure the account_name exactly matches your environment-variables and key vault. "
                f"For example, if your FBA Seller Account is 'Sell Stuff', make sure you pass initials 'ss' to the "
                f"'account_name' query parameter, that your key vault is titled 'ss-kv', and that the env-variable is "
                f"'TS_VAULT_NAME'."
            )
            raise
    ...
    def get_report(self, report_type: str, start_date: str, end_date: str, max_retries: int = 10) -> str:
        """
        ...
        current_attempt = 1
        max_attempts = max_retries  # these reports can be tiny/huge - setting higher max_attempts helps to scale
        while current_attempt <= max_attempts:
            try:
                ...
                if status == 'DONE':
                    ...
                    if report_type == 'GET_FBA_MYI_UNSUPPRESSED_INVENTORY_DATA':
                        ...
                        logging.info(f"new report ID is {self.GenerateFBAReport.report_id}")
                        self.GenerateFBAReport.get_download_url()
                        df = self.GenerateFBAReport.download_report()
                        df = df.to_json(orient='records')
                        logging.info("Successfully fetched the backup inventory report")
                        return df                        
